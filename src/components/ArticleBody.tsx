    ...
    <main className="min-h-screen bg-primary px-4 pb-12">
      <div className="mx-auto max-w-[1200px]">
        ...
        <section>
          ...
          <h2 className="mb-8 text-center text-3xl font-semibold text-gray-600 dark:text-text-primary">
          ...
          <div className="article mb-10 w-full">
            <p>
              ...
              smaller, more manageable steps. For example, in an online shopping
              website when a customer searches for a product, the platform can
              send a request to the product search API, then send a request to
              the product details API to retrieve more information about the
              products. In this article, we will{" "}
              <strong>create a custom plugin</strong> for
              <a href="">Apache APISIX</a> API Gateway to handle client requests
              that should be called in sequence.
            </p>
            ...
            <p>
              ...
              seem similar, a pipeline request involves sending a single request
              to the server that triggers a sequence of API requests to be
              executed in a defined order. Each API request in the sequence can
              modify the request and response data, and the response from one
              API request is passed as input to the next API request in the
              sequence. Pipeline requests can be useful when a client needs to
              execute a sequence of dependent API requests that must be executed
              in a specific order. In each step of the pipeline, we can
              transform or manipulate the response data before passing it on to
              the next step. This can be useful in situations where data needs
              to be normalized or transformed or filter out sensitive data
              before it is returned to the client. It can help to reduce overall
              latency. For example, one API call can be made while another is
              waiting for a response, reducing the overall time needed to
            ...
            <p>
              ...
              additional configuration with a <a href="">Docker compose.yml</a>{" "}
              file.
            </p>
            <h1>Prerequisites</h1>
            <ul>
              ...
              <li>
                <a href="">curl</a> is used to send requests to APISIX Admin
                API. You can also use easy tools such as <a href="">Postman</a>{" "}
    ...
    "plugins":{
        "pipeline-request":{
          "nodes":[
            ...
            <h1>Summary</h1>
            <p>
              Up to now, we learned that our custom pipeline request plugin for
              the Apache APISIX API Gateway allows us to define a sequence of
              API calls as a pipeline in a specific order. We can also use this
              new plugin with the combination of existing ones to enable
