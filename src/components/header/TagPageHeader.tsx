...
const TagPageHeader: FC<Props> = ({ tagDetails, following, followTag }) => {
  return (
    <header className="mb-4 flex flex-col items-center justify-center rounded-md border border-border-light bg-white px-6 py-8 text-center dark:border-border dark:bg-primary md:py-10 lg:py-11">
      <div className="flex gap-3">
        {tagDetails.logo ? (
          ...
          <div className="flex h-12 w-12 items-center justify-center rounded-md border border-slate-300 bg-gray-200 dark:border-slate-600 dark:bg-primary-light">
      ...
      <div className="mb-5 flex w-full flex-col items-center justify-center gap-4 md:flex-row">
        ...
        <Link
          href={`/article/new?tag=${tagDetails.name}`}
      ...
      <div className="flex flex-wrap items-center justify-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            href={`https://twitter.com/intent/tweet?text=${env.NEXT_PUBLIC_VERCEL_URL}/tag/${tagDetails.slug}`}
            target="_blank"
            rel="noopener noreferrer"
          >
            <button className="btn-icon-large flex">
              <Twitter className="h-5 w-5 fill-gray-700 stroke-none dark:fill-text-secondary" />
            </button>
          </a>
