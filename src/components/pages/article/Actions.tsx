...
const ArticleActions: FC<Props> = ({
  ...
  const [optionsControl, setOptionsControl] = useState<HTMLDivElement | null>(
    null,
  );
  const [optionsDropdown, setOptionsDropdown] = useState<HTMLDivElement | null>(
    null,
  );
      ...
      <div className="mx-auto flex items-center justify-between gap-2 rounded-full border border-border-light bg-light-bg px-4 py-1 shadow-sm dark:border-border dark:bg-primary-light sm:w-max">
        ...
        <Tooltip label="Bookmark" withArrow>
          <button
            ...
            role="button"
            className={`dark:hover:bg-border ${
...
>(({ setOptionsOpen, slug, user }, ref) => {
  ...
  const { mutateAsync: deleteArticle } =
    api.posts.deleteTemporarily.useMutation();
  const { mutateAsync: disableComment } =
    api.posts.disableComments.useMutation();
  const userActions = [
    "Edit",
    "Delete",
    "Disable Comments",
  ...
  const guestActions = ["Report", "Follow"];
