...
const CommentsModal: FC<{
  ...
  const {
    ...
    isLoading,
    refetch,
  } = api.comments.getComments.useQuery(commentState, {
    enabled: !!id,
      ...
      setCommentsData((prev) => ({
        ...
        comments: prev.comments.map((e) => {
          if (e.id === commentState.parentId) {
            return (
              comments?.comments.find((e) => e.id === commentState.parentId) ??
              e
            );
          } else {
            return e;
  ...
  return (
    <>
      ...
      <section
        ref={commentSection}
        className={`fixed right-0 top-0 h-full min-h-[100dvh] w-full max-w-[450px] overflow-auto border-l border-border-light bg-light-bg dark:border-border dark:bg-primary ${
        ...
        <section className="h-max">
          {isLoading && commentState.type === "INITIAL" ? (
            ...
            commentsData.comments.map((comment, index) => {
              ...
              return (
                <div
                  className={`relative border-b border-border-light px-4 py-3 last:border-none dark:border-border`}
                  key={comment.id}
                >
                  <CommentCard
                  ...
                  {(comment.repliesCount > 1 ||
                    (comment.replies[0]?.repliesCount !== undefined &&
                      comment.replies[0]?.repliesCount > 0)) &&
                    repliesButtonRef.current[index] == false && (
                      <button
                        onClick={() => {
